...
import { addSentenceCard, Card, removeSentenceCard, updateSentenceCard } from '@store/reducer/vocaReducer';

const ListSentenceCard = () => {
    const dispatch: AppDispatch = useDispatch();
    const { sentences } = useSelector((state: RootState) => state.vocabularies);

    const handleAddCard = (): void => {
        dispatch(addSentenceCard());
    };

    const handleUpdateCard = (index: number, english: string, vietnamese: string): void => {
        dispatch(updateSentenceCard({ index, english, vietnamese }));
    };

    const handleRemoveCard = (index: number): void => {
        dispatch(removeSentenceCard(index));
    };
    ...
    return (
        <div>
            ...
            <ul className="mt-4 flex flex-col gap-5">
                {sentences.map((card: Card, index: number) => (
...
export default ListSentenceCard;

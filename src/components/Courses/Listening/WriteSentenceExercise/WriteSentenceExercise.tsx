...
const WriteSentenceExercise = () => {
    ...
    const handleSubmit = (_id: string, english: string, vietnamese: string, indexAnswer: number) => {
        ...
        const correctVietnameseAnswer: boolean =
            removeNonLetters(vietnameseRemoveTone).trim() === removeNonLetters(answerRemoveTone).trim();

        const correctEnglishAnswer: boolean = removeNonLetters(englishRemoveTone).trim() === removeNonLetters(answerRemoveTone).trim();
